 ...
 */
function getArticlesByYearAndMonth(year = getYear() , month = 0 ){
  let arr = [];
  for ( let i = 0 ; i < articles.length ; i++ ){
    if ( getYear(articles[i].date) === year ){
      if ( month ){
...
export function getAllCategory(){
  let arr = [];
  for ( let i = 0 ; i < articles.length ; i++ ){
    if ( !arr.includes(articles[i].categories) ){
      arr.push(articles[i].categories);
    }
  }
  return arr;
};
...
function getYearCategory(year = getYear() , categories = getAllCategory()){
  ...
  for ( let i = 0 ; i < yearsArticles.length ; i ++ ){
    arr[categories.indexOf(yearsArticles[i].categories)]++;
  }
  return arr;
}
/**
...
function getArticlesByCategory(category){
  let arr = [];
  for ( let i = 0 ; i < articles.length ; i++ ){
    if ( articles[i].categories === category ){
      arr.push(articles[i]);
    }
  }
  return arr;
...
export function getAuthorArticles(){
  ...
  for ( let j = 0 ; j < authors.length ; j++ ){
    ...
    for ( let i = 0 ; i < months.length ; i++ ){
      ...
      let articles = getArticlesByYearAndMonth(months[i].year , months[i].month);
      for (let k = 0 ; k < articles.length ; k++ ){
        if ( articles[k].author === authors[j] ){
...
export function getAuthorCategory(){
  ...
  for ( let i = 0 ; i < authors.length ; i++ ){
    ...
    for ( let j = 0 ; j < categories.length ; j++ ){
      ...
      let articles = getArticlesByCategory(categories[j]);
      for ( let k = 0 ; k < articles.length ; k++ ){
        if ( articles[k].author === authors[i] ){
